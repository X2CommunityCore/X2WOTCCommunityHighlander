...
var float fFreeMovementInterpTime;		// Issue #999
...
function InitializeFor(PlayerController PC)
{
	...
	CurrentZoom = 1.0f;
	fFreeMovementInterpTime=1.0f;		// Issue #999
}
...
function FocusOnFacility(name RoomName)
{
	...
	StartRoomView('FreeMovement', fFreeMovementInterpTime);			//Issue #999
...
// Start Issue #999
// Overrides XComBaseCamera::SetCameraState
// This override bypasses the hardcoded interpolation value in XComHeadquartersCamera::StartRoomView
function XComCameraState SetCameraState( class<XComCameraState> NewStateClass, float InInterpTime )
{
    ...
    {
        InInterpTime = fFreeMovementInterpTime;
    }
	class'X2TacticalGameRuleset'.static.ReleaseScriptLog( "*****FREE INTERP TIME: "@InInterpTime@"*****" );
