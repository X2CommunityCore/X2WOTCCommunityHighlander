...
simulated function UpdateAbilityAvailability(out AvailableAction Action)
{
	...
	if (kUnit != none)
	{
		...
		if (Action.AvailableCode == 'AA_Success')
		{
			...
			// Note: Cheap short-circuiting
			if (class'CHProfiler'.default.bAbilityProfileEnabled
				&& class'CHProfiler'.static.ShouldExtendedProfileActivity(GetMyTemplateName()))
			{
				Action.AvailableCode = CHGatherAbilityTargetsScript(Action.AvailableTargets);
			}
			else
			{
