...
function AddEvent(name Event, delegate<X2EventManager.OnEventDelegate> EventFn, EventListenerDeferral Deferral = ELD_OnStateSubmitted)
{
...
function RegisterForEvents()
{
	...
	foreach EventsToRegister(EventPair)
	{
		...
		{
			// Issue #4 Allow to specify EventListenerDeferral for X2EventListenerTemplates
			EventManager.RegisterForEvent(selfObject, EventPair.EventName, EventPair.Callback, EventPair.Deferral);
		}
	}
}
