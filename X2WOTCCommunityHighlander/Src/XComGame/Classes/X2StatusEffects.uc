...
{
	// Start Issue #91:
	// Add Tuple Object and Event Trigger to allow effects to modify Bleedout Chance
	local int			BleedoutChance;
	local XComLWTuple	Tuple;

	BleedoutChance = UnitState.GetCurrentStat(eStat_Will) - default.BLEEDOUT_BASE;

	// Set up a Tuple to pass the overkill damage, and pass/return the Bleedout Chance
	...
	// To use, register the XComGameState_Effect for the event, pull in the Tuple, and manipulate the standard Bleedout Chance in Tuple.Data[0].i as desired.
	// This function will then pass on whatever the new value of Tuple.Data[0].i may be instead of the standard Bleedout Chance. This chance gets rolled
	// against BLEEDOUT_ROLL in XComGameCore.ini [default: 100] or BLEEDOUT_BONUS_ROLL [default: 50] if Stay With Me is unlocked.
	// NOTE: OverkillDamage isn't actually used as part of the Bleedout Chance calculation or rolls by default, though the value is passed from the TakeDamage
	// event and can be used to modify the Bleedout Chance accordingly in mod-added EventListenerReturn functions, if desired.
