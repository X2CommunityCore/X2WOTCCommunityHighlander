...
function XComGameState_Unit CreateCharacter(XComGameState StartState, optional ECharacterPoolSelectionMode SelectionModeOverride = eCPSM_None, optional name CharacterTemplateName, optional name ForceCountry, optional string UnitName )
{
	...
	else
	{
		...
		`assert(CharacterGenerator != none);
		// Issue #70 Start
		// Since the optional parameter gender comes before the country select gender with a random chance of male1/female2
		CharacterGeneratorResult = CharacterGenerator.CreateTSoldier(CharacterTemplateName, EGender(`SYNC_RAND( 1 ) + 1), forceCountry);
		// Issue #70 End
