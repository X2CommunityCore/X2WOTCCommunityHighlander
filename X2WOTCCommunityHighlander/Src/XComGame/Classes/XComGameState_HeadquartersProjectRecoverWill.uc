...
function int CalculateWorkPerHour(optional XComGameState StartState = none, optional bool bAssumeActive = false)
{
	...
	local XComLWTuple Tuple; // Issue #650
	local float Modifier; // Issue #650
	...
	Tuple.Data[0].f = Modifier;

	`XEVENTMGR.TriggerEvent('WillRecoveryTimeModifier', Tuple, self);

	Modifier = Tuple.Data[0].f;

	// Issue #650 End

	return Round(float(OriginalWorkPerHour) * ResHQ.GetWillRecoveryRateScalar() * Modifier); // Issue #650 - added modifier to calculation
